...
{
    private const TYPES = ['text','integer','decimal','date','time','yes_no','select','multi_select','photo','signature','qr','barcode','formula','section'];
    ...
    public function preview(DailyReportForm $dailyReportForm)
    {
        ...
        return view('daily-reports.fill', ['form' => $dailyReportForm, 'preview' => true]);
    }
    ...
    public function fill(DailyReportForm $dailyReportForm)
    {
        ...
        $dailyReportForm->load('fields');
        return view('daily-reports.fill', ['form' => $dailyReportForm, 'preview' => false]);
    }
    ...
    public function submit(Request $request, DailyReportForm $dailyReportForm)
    {
        ...
        foreach ($dailyReportForm->fields as $field) {
            ...
            $rules["responses.{$field->field_key}"] = match ($field->type) {
                'integer' => [...$base, 'integer'],
                'decimal','formula' => [...$base, 'numeric'],
                'date' => [...$base, 'date'],
                'photo' => [...$base, 'file', 'image', 'max:10240'],
                'multi_select' => [...$base, 'array'],
    ...
    private function validatedForm(Request $request): array
    {
        $data = $request->validate([
            ...
            'fields.*.options'=>'nullable|string','fields.*.formula'=>'nullable|string|max:500','user_ids'=>'nullable|array','user_ids.*'=>'exists:users,id',
        ]);
    ...
    private function saveFields(DailyReportForm $form, array $fields): void
    {
        ...
        foreach ($fields as $position => $field) {
            ...
            $form->fields()->create([
                ...
                'formula'=>$field['formula'] ?? null,'is_required'=>filter_var($field['is_required'] ?? false, FILTER_VALIDATE_BOOL),
