...
class UserController extends Controller
{
    ...
    public function store(Request $request) { $this->allowed(); $data=$request->validate(['name'=>'required|max:255','email'=>'required|email|unique:users','password'=>'required|min:8','branch'=>'required|max:255','profile'=>'required|max:100','permissions'=>'required|array|min:1','permissions.*'=>'in:products,requirements,approvals,logistics,daily-reports,users']); $data['is_active']=$request->boolean('is_active'); User::create($data); return back()->with('success','Usuario creado correctamente.'); }
    public function update(Request $request, User $user) { $this->allowed(); $data=$request->validate(['name'=>'required|max:255','email'=>['required','email',Rule::unique('users')->ignore($user->id)],'password'=>'nullable|min:8','branch'=>'required|max:255','profile'=>'required|max:100','permissions'=>'required|array|min:1','permissions.*'=>'in:products,requirements,approvals,logistics,daily-reports,users']); if(empty($data['password'])) unset($data['password']); $data['is_active']=$request->boolean('is_active'); $user->update($data); return back()->with('success','Usuario actualizado.'); }
}
