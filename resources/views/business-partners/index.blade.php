...
<dialog class="bank-management-dialog" id="bank-management"><div class="modal-head product-modal-head"><span class="modal-icon">▤</span><div><h2>Bancos y cuentas de proveedores</h2><p>Registra bancos padre y cuentas en soles o dólares. Los códigos se generan automáticamente.</p></div><button type="button" data-close>×</button></div><div class="bank-management-grid"><section><h3>Bancos</h3><form action="{{ route('banks.store') }}" class="bank-inline-form" id="bank-form">@csrf<input name="name" placeholder="Nombre del banco" required><button class="secondary">＋ Agregar banco</button></form><div class="bank-list" id="bank-list">@forelse($banks as $bank)<div><strong>{{ $bank->name }}</strong><small>{{ $bank->code }} · {{ $bank->accounts_count }} cuenta(s)</small></div>@empty<p>Aún no hay bancos registrados.</p>@endforelse</div></section><section><h3>Cuentas de proveedores</h3><form action="{{ route('business-partners.bank-accounts.store') }}" class="bank-account-form" id="bank-account-form">@csrf<label>Proveedor *<select name="business_partner_id" required><option value="">Seleccionar proveedor</option>@foreach($suppliers as $supplier)<option value="{{ $supplier->id }}">{{ $supplier->document_number }} · {{ $supplier->name }}</option>@endforeach</select></label><label>Banco *<select name="bank_id" id="bank-account-bank" required><option value="">Seleccionar banco</option>@foreach($banks as $bank)<option value="{{ $bank->id }}">{{ $bank->name }}</option>@endforeach</select></label><label>Tipo de cuenta *<select name="account_type"><option value="Cuenta Corriente">Cuenta Corriente</option><option value="Cuenta Interbancaria">Cuenta Interbancaria</option></select></label><label>Moneda *<select name="currency"><option value="PEN">Soles (PEN)</option><option value="USD">Dólares (USD)</option></select></label><label>Número de cuenta *<input name="account_number" required></label><label>Titular<input name="holder_name"></label><button class="primary">Guardar cuenta</button><small class="bank-form-status" id="bank-account-status"></small></form></section></div><div class="bank-accounts-table"><h3>Cuentas registradas</h3><div class="table-wrap"><table><thead><tr><th>Proveedor</th><th>Banco</th><th>Tipo</th><th>Moneda</th><th>Cuenta</th><th>Titular</th></tr></thead><tbody id="bank-accounts-body">@forelse($bankAccounts as $account)<tr><td>{{ $account->partner?->name ?: 'Sin proveedor' }}</td><td>{{ $account->bank?->name ?: $account->bank_name }}</td><td>{{ $account->account_type }}</td><td>{{ $account->currency === 'USD' ? 'Dólares' : 'Soles' }}</td><td><code>{{ $account->account_number }}</code></td><td>{{ $account->holder_name ?: '—' }}</td></tr>@empty<tr data-empty><td colspan="6">Aún no hay cuentas registradas.</td></tr>@endforelse</tbody></table></div></div><div class="modal-foot"><button type="button" data-close>Cerrar</button></div></dialog>
@endsection
@push('scripts')
<script>
(() => {
    ...
    management.querySelector('#bank-form').addEventListener('submit', async event => { event.preventDefault(); const form=event.currentTarget; try { const bank=await requestJson(form); const list=management.querySelector('#bank-list'); list.querySelector('p')?.remove(); list.insertAdjacentHTML('afterbegin',`<div><strong>${escape(bank.name)}</strong><small>${escape(bank.code)} · 0 cuenta(s)</small></div>`); management.querySelector('#bank-account-bank').insertAdjacentHTML('beforeend',`<option value="${bank.id}" selected>${escape(bank.name)}</option>`); form.reset(); } catch(error) { alert(error.message); } });
    management.querySelector('#bank-account-form').addEventListener('submit', async event => { event.preventDefault(); const form=event.currentTarget, status=management.querySelector('#bank-account-status'); status.textContent='Guardando cuenta…'; try { const account=await requestJson(form); const body=management.querySelector('#bank-accounts-body'); body.querySelector('[data-empty]')?.remove(); body.insertAdjacentHTML('afterbegin',`<tr><td>${escape(account.partner)}</td><td>${escape(account.bank)}</td><td>${escape(account.type)}</td><td>${account.currency==='USD'?'Dólares':'Soles'}</td><td><code>${escape(account.number)}</code></td><td>${escape(account.holder || '—')}</td></tr>`); form.reset(); status.textContent='✓ Cuenta agregada correctamente.'; } catch(error) { status.textContent=error.message; } });
})();
</script>
@endpush
