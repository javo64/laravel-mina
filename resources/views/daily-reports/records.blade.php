...
function initFallbackRecordsMap(){if(!recordPoints.length)return;centralMap=L.map('records-map').setView([recordPoints[0].lat,recordPoints[0].lng],16);L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{maxZoom:20,attribution:'Imágenes © Esri'}).addTo(centralMap);const bounds=[];recordPoints.forEach(point=>{const marker=L.marker([point.lat,point.lng]).addTo(centralMap).bindPopup(markerContent(point));centralMarkers[point.id]={marker};bounds.push([point.lat,point.lng])});centralMap.fitBounds(bounds,{padding:[30,30],maxZoom:18})}
document.querySelectorAll('.locate-report').forEach(button=>button.onclick=()=>{const item=centralMarkers[button.dataset.id];if(!item)return;if(window.google?.maps){centralMap.panTo(item.marker.getPosition());centralMap.setZoom(19);item.info.open({anchor:item.marker,map:centralMap})}else{centralMap.setView(item.marker.getLatLng(),19);item.marker.openPopup()}document.querySelector('.records-map-panel').scrollIntoView({behavior:'smooth',block:'center'})});
