...
<div class="product-form">
    ...
    <label class="span-4">Nombre secundario<input name="secondary_name" value="{{ old('secondary_name',optional($product)->secondary_name) }}" placeholder="Nombre alternativo"></label>
    <label class="span-8">Descripción<textarea name="description" rows="2" placeholder="Añade una breve descripción...">{{ old('description',optional($product)->description) }}</textarea></label>
    <label class="span-4">Categoría<select name="category"><option value="">Seleccionar categoría</option>@foreach($categories as $category)<option value="{{ $category->name }}" {{ $selectedCategory===$category->name?'selected':'' }}>{{ $category->name }}</option>@endforeach</select></label>
